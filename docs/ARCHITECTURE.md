...
Memory Glass は、2つのレビューモードを提供しています。

#### ① 📋 一覧モード
登録されている全カードをスクロール閲覧しながら、自由に反復学習を行える従来のモードです。

#### ② 🧠 定着学習モード
SM-2アルゴリズムに基づき、記憶定着のために復習が必要な優先度の高いものから順に1枚ずつ美しいカードアニメーション（フェードイン・スライドアップ）付きで学習を進めるモードです。
- ナビゲーション（◀ 前へ、次へ ▶）により、学習済みカードの見直しも可能です。
- カードの評価（Forgot, Hard, Good, Easy）が選択されると、アルゴリズムによる更新が行われ、0.6秒後に自動で次のカードへとフォーカスが移ります。
